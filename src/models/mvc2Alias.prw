...
Static Function ModelDef()
    ...
    Local oStru${3:alias2} := FwFormStruct(1, "${4:alias2}")
    Local oModel := Nil

    oModel := MpFormModel():New("")

    oModel:AddFields("${2:alias1}MASTER",, oStru${2:alias1})
    oModel:AddGrid("${4:alias2}DETAIL", "${2:alias1}MASTER", oStru${4:alias2})

    oModel:SetRelation("${4:alias2}DETAIL", { ;
        {"FIELD_${2:alias1}", "FIELD_${4:alias2}"} ;
    }, ${3:alias2}->(IndexKey(1)))

    oModel:SetPrimaryKey({})

    oModel:SetDescription("")
    oModel:GetModel("${2:alias1}MASTER"):SetDescription("${3:descriptionAlias1}")
    oModel:GetModel("${4:alias2}DETAIL"):SetDescription("${5:descriptionAlias2}")
Return oModel


Static Function ViewDef()
    Local oStru${2:alias1} := FwFormStruct(2, "${2:alias1}")
    Local oStru${3:alias2} := FwFormStruct(2, "${4:alias2}")
    ...
    oView:AddGrid("VIEW_${4:alias2}", oStru${3:alias2}, "${4:alias2}DETAIL")

    oView:CreateHorizontalBox("CABEC", 50)
    oView:CreateHorizontalBox("GRID", 50)

    oView:SetOwnerView("VIEW_${2:alias1}", "CABEC")
    oView:SetOwnerView("VIEW_${4:alias2}", "GRID")
Return oView
